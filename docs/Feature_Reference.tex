...
%\newfontface\condbold{Junicode Two Beta Condensed Bold}
...
cv62.}\\[1ex]
{\condbold\addfontfeature{Language=Polish} Mieszkańcy całej ziemi mieli jedną mowę, czyli jednakowe słowa.  A
...
and Coptic” range (U+0370–U+03FF). When monotonic text is set in all caps, Junicode
...
(U+1FBB), not \textLetterExample{Ά} (U+0386),
...
to right or bottom to top; 3. the \textit{ypogegrammeni} (U+0345), or for
capitals, if you prefer, the \textit{prosgegrammeni} (U+1FBE).

For example, the sequence ω (U+03C9) ◌̓ (U+0313) ◌́ (U+0301) ◌ͅ (U+0345)
produces \textLetterExample{\grk \char"03C9\char"0313\char"0301\char"0345}.
Substitute capital Ω (U+03A9) and the result is
\textLetterExample{\grk \char"03A9\char"0313\char"0301\char"0345}. Note that in a number of applications the layout
...
(Without U+034F, you would get \textLetterExample{\cvd[1]{84}{ū}}).
...
\subsection[47. cv86 {}-- Variant of ◌ᷘ U(U+1DD8, combining insular d)]{%
...
%\subsection[51. cv90 {}-- Variants of ◌̄  (U+0304, combining macron)]{\stepcounter{Feature}{\theFeature}.
...
Without U+034F: \textLetterExample{\addfontfeature{StylisticSet=17}aō}.
...
\vspace*{\fill}
\begin{center}
{\stditalic{This document was set in 12pt Junicode VF SemiExpanded\\
using the {\ltech} typesetting system with fontspec for font management.\\
