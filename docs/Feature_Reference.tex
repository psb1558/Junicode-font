...
%\newcommand\textstyleUName[1]{\textrm{#1}}
...
All such characters (those with hexadecimal codes that \textstyleEmphasis{do not} begin with \textstyleletterexample{E}
or \textstyleletterexample{F}) are presumed safe to use in accessible and searchable text. However, some of these are
...
diacritics. For example, instead of MUFI U+E498 \textstyleUName{latin small letter e with dot below and acute}, use
\textstyleletterexample{e} + U+0323 \textstyleUName{combining dot below} + U+0301 \textstyleUName{combining acute accent}:
\textstyleletterexample{ẹ́} (when applying combining marks, start with any marks below the character and work
downwards, then continue with any marks above the character and work upwards. For example, to make
\textstyleletterexample{ǭ̣́}, place characters in this order: \textstyleletterexample{o},
\textstyleUName{combining ogonek} U+0328, \textstyleUName{combining dot below} U+0323, \textstyleUName{combining
macron} U+0304, \textstyleUName{combining acute} U+0301). Some MUFI characters have marks in unconventional locations,
e.g. \textstyleletterexample{ȯ́} \textstyleUName{latin small letter o with dot above and acute}, where the
acute appears beside the dot instead of above. This and other characters like it should still be entered as a sequence
of base character + marks (here \textstyleletterexample{o}, \textstyleUName{combining dot above} U+0307,
\textstyleUName{combining acute} U+0301). Junicode will position the marks correctly.
...
\begin{itemize}
\item  U+E8AF. This is a ligature of long \textstyleletterexample{s} and \textstyleletterexample{l} with stroke,
but there are no base characters with this style of stroke.
\item \textstyleletterexample{} U+EFD8 and U+EFD9. MUFI lists these as ligatures (corresponding to the
historic ligatures \textstyleletterexample{\hlig{uuUU}}, but they cannot be treated as ligatures in the
font because a single diacritic is positioned over the glyphs as if they were digraphs like
\textstyleletterexample{ꜳꜲ}.
\item \textstyleletterexample{} U+EBE7 and U+EBE6, for the same reason.
\item \textstyleletterexample{} U+F159 \textstyleUName{latin abbreviation sign small de}. Neither a variant of
\textstyleletterexample{d} nor an eth (\textstyleletterexample{ð}), this character may be a candidate for Unicode
encoding.
\end{itemize}
...
\liststyleLii
\begin{itemize}
\item U+F1C5 \textstyleUName{combining curl high position}. Use U+1DCE \textstyleUName{combining ogonek above}. The
positioning problem mentioned in the MUFI recommendation is solved in Junicode (and, to be fair, many other fonts with
OpenType programming).
\item U+F1CA \textstyleUName{combining dot above high position}. Use U+0307 \textstyleUName{combining dot above}. It
...
\textstyleSourceText{smcp} -- Small Capitals}
...
characters using combining diacritics, as outlined in the introduction. For example, \textstyleSourceText{smcp} applied
to the sequence \textstyleletterexample{t} + \textstyleUName{combining ogonek} (U+0328) + \textstyleUName{combining
acute} (U+0301) will change \textstyleletterexample{t̨́} to \textstyleletterexample{\textsc{t̨́}}.

\subsection[2. c2sc {}-- Small Capitals from Capitals]{\stepcounter{Feature}{\theFeature}. \textstyleSourceText{c2sc} --
Small Capitals from Capitals}
Use with \textstyleSourceText{smcp} for all-small-cap text. ABCDE $\rightarrow $ {\addfontfeature{Letters = UppercaseSmallCaps}ABCDE}.

\subsection[3. pcap {}-- Petite Capitals]{\stepcounter{Feature}{\theFeature}. \textstyleSourceText{pcap} -- Petite
Capitals}
Produces small caps in a smaller size than \textstyleSourceText{smcp}. Use these when small caps have to be mixed with
...
\subsection[4. case {}-- Case{}-Sensitive Forms]{\stepcounter{Feature}{\theFeature}. \textstyleSourceText{case} --
...
\textstyleSourceText{nalt} -- Alternate Annotation Forms}
Produces letters and numbers circled, in parenthesis, or followed by periods, as follows:

\textstyleSourceText{nalt[1]}, circled letters or numbers: {\addfontfeature{Annotation=0}a b .~.~. z; 0 1 2 .~.~. 20}.

\textstyleSourceText{nalt[2]}, letter or numbers in parentheses: {\addfontfeature{Annotation=1}a .~.~. z; 0 1 .~.~. 20}.

\textstyleSourceText{nalt[3]}, double-circled numbers: {\addfontfeature{Annotation=2}0 1 .~.~. 10}.

\textstyleSourceText{nalt[4]}, white numbers in black circles: {\addfontfeature{Annotation=3}0 1 2 3 . . . 20}.

\textstyleSourceText{nalt[5]}, numbers followed by period: {\addfontfeature{Annotation=0}0 1 . . . 20}.

\noindent For enclosed figures 10 and higher, \textstyleSourceText{rlig} (Required Ligatures) must also be enabled (as it should
be by default: see \hyperlink{req}{Required Features} below).

\subsection[6. tnum {}-- Tabular Figures]{\stepcounter{Feature}{\theFeature}. \textstyleSourceText{tnum} -- Tabular
Figures}
Fixed-width figures: 0123456789 (default or with \textstyleSourceText{lnum}), {\addfontfeature{Numbers=OldStyle}0123456789} (with
\textstyleSourceText{onum}).

\subsection[7. onum {}-- Oldstyle Figures]{\stepcounter{Feature}{\theFeature}. \textstyleSourceText{onum} -- Oldstyle
Figures}
Figures that harmonize with lowercase characters: {\addfontfeature{Numbers=OldStyle}0123456789} (default or with
\textstyleSourceText{tnum}), {\addfontfeature{Numbers={Proportional,OldStyle}}0123456789}
(with \textstyleSourceText{pnum}). When combined with \textstyleSourceText{pnum}, this feature also affects subscripts
and superscripts.

\subsection[8. pnum {}-- Proportional Figures]{\stepcounter{Feature}{\theFeature}. \textstyleSourceText{pnum} --
Proportional Figures}
Proportionally spaced figures: {\addfontfeature{Numbers=Proportional}0123456789} (default or with \textstyleSourceText{lnum}),
{\addfontfeature{Numbers={Lowercase,Proportional}}0123456789} (with
\textstyleSourceText{onum}). When combined with \textstyleSourceText{onum}, this feature also affects subscripts and
superscripts. Most applications (including MS Word) with any support of OpenType features will support this feature and
\textstyleSourceText{lnum} in such a way that you don't have to enter them manually.

\subsection[9. lnum {}-- Lining Figures]{\stepcounter{Feature}{\theFeature}. \textstyleSourceText{lnum} -- Lining
Figures}
Figures in a uniform height, harmonizing with uppercase letters: 0123456789 (default or with
\textstyleSourceText{tnum}), {\addfontfeature{Numbers=Proportional}0123456789} (with \textstyleSourceText{pnum}).

\subsection[10. zero {}-- Slashed Zero]{\stepcounter{Feature}{\theFeature}. \textstyleSourceText{zero} -- Slashed Zero}
...
\textstyleSourceText{sups} -- Superscripts}
...
superscripts are needed (e.g. for footnote numbers), use \textstyleSourceText{sups} instead of the raised and scaled
characters generated by some programs. With sups: \sups{4567}. Scaled: \textsuperscript{4567}.

\subsection[12. subs {}-- Subscripts]{\stepcounter{Feature}{\theFeature}. \textstyleSourceText{subs} -- Subscripts}
Produces subscript numbers. Only affects lining tabular (the default numbers) and oldstyle proportional figures (use
\textstyleSourceText{pnum} and \textstyleSourceText{onum}): \subs{8901 \oprop{2345}}.

\section{D. Ornaments}
\hypertarget{SectionD}{}\subsection[13. ornm {}-- Ornaments]{\stepcounter{Feature}{\theFeature}.
\textstyleSourceText{ornm} -- Ornaments}
...
when text is displayed in an environment where Junicode is not available or \textstyleSourceText{ornm} is not
implemented.
...
\textstyleSourceText{cv01-cv52} -- Basic Latin Variants}
These features also affect small cap (\textstyleSourceText{smcp}) and underdotted (\textstyleSourceText{ss07}) forms,
where available. Where Junicode has no variant for a Basic Latin letter, the expected \textstyleSourceText{cvNN}
feature is skipped, being reserved for future development. These features also affect small cap
(\textstyleSourceText{smcp}) and underdotted (\textstyleSourceText{ss07}) forms, where available.
...
\textstyleSourceText{cv53-cv67} -- Other Latin Letters}
\hypertarget{OtherLatin}{}Some features affect both upper- and lowercase forms. \textstyleSourceText{cv62} also affects
combining \textstyleletterexample{e} with ogonek, accessible via \hyperlink{ss10}{\textstyleSourceText{ss10}} with the
entity reference \textstyleSourceText{\&\_eogo;}. In this range, \textstyleSourceText{cvNN} features are not reserved
for future development, since Junicode already uses or reserves all of the available \textstyleSourceText{cvNN}
features.
...
\centering ꜳ (U+A733) &
\centering cv55 &
\centering\arraybslash{\mdseries 1=\cvd{55}{ꜳ}, 2=\cvd{55}{ꜳ}}\\\hline
...
\centering ꝥ, {\addfontfeature{StylisticSet=1}ꝥ} (U+A765) &
\centering cv67 &
\centering\arraybslash 1=\cvd{67}{ꝥ, {\addfontfeature{StylisticSet=1}ꝥ}}\\\hline
\end{supertabular}
\end{center}
\subsection[16. ss01 {}-- Alternate thorn and eth]{\stepcounter{Feature}{\theFeature}. \textstyleSourceText{ss01} --
...
\hyperlink{OtherLatin}{\textstyleSourceText{cv67}}), combining mark eth
(U+1DD9, ◌ᷙ \revthorn{◌ᷙ}), and enlarged thorn and eth (see \hyperlink{ss06}{\textstyleSourceText{ss06}}).
This feature depends on \hyperlink{req}{\textstyleSourceText{loca}} (Localized Forms), which in most applications will
always be enabled.

\subsection[17. ss02 {}-- Insular Letter{}-Forms]{\stepcounter{Feature}{\theFeature}. \textstyleSourceText{ss02} --
...
(\textstyleSourceText{cvNN}) features.

\subsection[18. hist {}-- Historical Forms]{\stepcounter{Feature}{\theFeature}. \textstyleSourceText{hist} -- Historical
Forms}
Changes \textstyleletterexample{s} to \textstyleletterexample{\addfontfeature{Style=Historic}s} (longs).

\subsection[19. ss03 {}-- Long s]{\stepcounter{Feature}{\theFeature}. \textstyleSourceText{ss03} -- Long s}
Changes \textstyleletterexample{s} to \textstyleletterexample{\addfontfeature{StylisticSet=3}s} (duplicating \textstyleSourceText{hist}). see also
\textstyleSourceText{ss08}.

\subsection[20. ss04 {}-- High Overline]{\stepcounter{Feature}{\theFeature}. \textstyleSourceText{ss04} -- High
Overline}
Produces a high overline over letters used as roman numbers: {\addfontfeature{StylisticSet=4}cdijlmvx CDIJLMVXↃ}.

\subsection[21. ss05 {}-- Medium{}-High Overline]{\stepcounter{Feature}{\theFeature}. \textstyleSourceText{ss05} --
...
\subsection[22. ss06 {}-- Enlarged Minuscules]{\stepcounter{Feature}{\theFeature}. \textstyleSourceText{ss06} --
Enlarged Minuscules}
...
\subsection[23. ss07 {}-- Underdotted Text]{\stepcounter{Feature}{\theFeature}. \textstyleSourceText{ss07} --
Underdotted Text}
...
\subsection[24. ss08 {}-- Contextual Long s]{\stepcounter{Feature}{\theFeature}. \textstyleSourceText{ss08} --
Contextual Long s}
In English and French text only, varies \textstyleletterexample{s} and \textstyleletterexample{\addfontfeature{Style=Historic}s} according to rules
...
feature to work properly, \textstyleSourceText{calt} ``Contextual Alternates'' must also be enabled (as it should be by
default: see \hyperlink{req}{Required Features} below).

\subsection[25. ss11 {}-- r Rotunda]{\stepcounter{Feature}{\theFeature}. \textstyleSourceText{ss11} -- r Rotunda}
\hypertarget{ss11}{}In lowercase and small caps, substitutes r rotunda (\textstyleletterexample{ꝛ}) for
\textstyleletterexample{r}. This features does not affect capital \textstyleletterexample{R}: the uncommon
\textstyleletterexample{Ꝛ} (U+A75A) must be entered manually. See also
\hyperlink{ss16}{\textstyleSourceText{ss16}}.

\subsection[26. ss16 {}-- Contextual r Rotunda]{\stepcounter{Feature}{\theFeature}. \textstyleSourceText{ss16} --
Contextual r Rotunda}
\hypertarget{ss16}{}Converts \textstyleletterexample{r} to \textstyleletterexample{ꝛ} (lowercase only) following the
...
\textstyleSourceText{calt} ``Contextual Alternates'' must also be enabled (as it should be by default: see
\hyperlink{req}{Required Features} below). See also \hyperlink{ss11}{\textstyleSourceText{ss11}}.

\subsection[27. cv68 {}-- Variant of ʔ (U+0294, glottal stop)]{\stepcounter{Feature}{\theFeature}.
\textstyleSourceText{cv68} -- Variant of ʔ (U+0294, glottal stop)}
1=\cvd{68}{ʔ}.

\subsection[28. cv99 {}-- revert small cap a to lowercase a]{\stepcounter{Feature}{\theFeature}.
\textstyleSourceText{cv}\textstyleSourceText{99} -- revert small cap a to lowercase a}
\hypertarget{RevertSmallCapA}{}1=a. This features reverts small cap
\textstyleletterexample{\textsc{a}} to \textstyleletterexample{a}, enabling it to
ligature with small cap \textstyleletterexample{\textsc{n}} or
\textstyleletterexample{\textsc{r}} via \textstyleSourceText{hlig}: \textsc{\cvd{99}{\hlig{an, ar}}}.
Be sure to apply \textstyleSourceText{smcp}, \textstyleSourceText{cv99} and
\textstyleSourceText{hlig} to both components of the ligature.
...
either of two ways: all are indexed variants of \textstyleletterexample{.} (period), and all are associated with the Unicode marks of
...
\textstyleSourceText{ss18} -- Old-Style Punctuation Spacing}
...
nota)]{\stepcounter{Feature}{\theFeature}. \textstyleSourceText{cv69} -- Variants of ⁊⹒
(U+204A / U+2E52, Tironian nota)}
1=\cvd{69}{⁊⹒}, 2=\cvd[1]{69}{⁊⹒}.

\subsection[31. cv70 {}-- Variants of . (period)]{\stepcounter{Feature}{\theFeature}. \textstyleSourceText{cv70} --
  Variants of . (period)}
...
\textstyleSourceText{cv71} -- Variant of {\textperiodcentered} (U+00B7, middle dot)}
1=\cvd{71}{\textperiodcentered} (\textit{distinctio}).

\subsection[33. cv72 {}-- Variants of , (comma)]{\stepcounter{Feature}{\theFeature}. \textstyleSourceText{cv72} --
Variants of , (comma)}
1=\cvd{72}{,}, 2=\cvd[1]{72}{,}.

\subsection[34. cv73 {}-- Variants of ; (semicolon)]{\stepcounter{Feature}{\theFeature}. \textstyleSourceText{cv73} --
...
\textstyleSourceText{cv74} -- Variants of ⹎ (U+2E4E, \textit{punctus elevatus})}
...
\textstyleSourceText{cv7}\textstyleSourceText{5} -- Variant of ! (exclamation mark)}
1=\cvd{75}{!} (\textit{punctus exclamativus}).

\subsection[37. cv76 {}-- Variants of ? (question mark)]{\stepcounter{Feature}{\theFeature}. \textstyleSourceText{cv76}
-- Variants of ? (question mark)}
1=\cvd{76}{?}, 2=\cvd[1]{76}{?}, 3=\cvd[2]{76}{?}. Shapes of the \textit{punctus interrogativus}.

\subsection[38. cv77 {}-- Variant of \~{} (ASCII tilde)]{\stepcounter{Feature}{\theFeature}. \textstyleSourceText{cv77}
-- Variant of \~{} (ASCII tilde)}
1=\cvd{77}{\~{}} (same as MUFI U+F1F9, ``wavy line'').

\subsection[39. cv78 {}-- Variant of * (asterisk)]{\stepcounter{Feature}{\theFeature}. \textstyleSourceText{cv78} --
Variant of * (asterisk)}
...
\textstyleSourceText{cv7}\textstyleSourceText{9} -- Variants of / (slash)}
...
abbreviation)]{\stepcounter{Feature}{\theFeature}. \textstyleSourceText{cv80} -- Variant of ꝝ (U+A75D, rum
abbreviation)}
1=\cvd{80}{ꝝ}.

\subsection[42. cv81 {}-- Variants of ◌͛ (U+035B, combining zigzag
above)]{\stepcounter{Feature}{\theFeature}. \textstyleSourceText{cv81} -- Variants of ◌͛ (U+035B, combining
zigzag above)}
1=\cvd{81}{◌͛}, 2=\cvd[1]{81}{◌͛}, 3=\cvd[2]{81}{◌͛}. Positioning of the zigzag can differ from that of other combining
marks, e.g. b͛, f͛, d͛. If \textstyleSourceText{calt} ``Contextual Alternates'' is enabled (as it is by
default in most apps), variant forms of \textstyleSourceText{cv81[2]} will be used with several letters, e.g. \cvd[1]{81}{d͛,
  f͛, k͛}. Enable \textstyleSourceText{case} for forms that harmonize with capitals
({\addfontfeature{Letters=Uppercase}\cvd[1]{81}{A͛ B͛ C͛ D͛}}), \textstyleSourceText{smcp} for forms that harmonize with small caps
  (\textsc{\cvd[1]{81}{e͛ f͛ g͛ h͛}}).

\subsection[43. cv82 {}-- Variants of spacing ꝰ (U+A770)]{\stepcounter{Feature}{\theFeature}.
\textstyleSourceText{cv82} -- Variants of spacing ꝰ (U+A770)}
1=\cvd{82}{ꝰ}, 2=\cvd[1]{82}{ꝰ}. \textstyleSourceText{cv82[1]} produces the baseline \textit{{}-us} abbreviation (same as MUFI
...
\textstyleSourceText{cv83} -- Variant of ꝫ (U+A76B, ``et'' abbreviation)}
...
U+1DD1)]{\stepcounter{Feature}{\theFeature}. \textstyleSourceText{cv84} -- MUFI combining marks (variants of U+1DD1)}
...
position them correctly over their base characters. To avoid these problems, enter U+1DD1 (◌᷑, \textstyleUName{combining ur
above}) and apply \textstyleSourceText{cv84}, with the appropriate index, to both mark and base character. This
collection of marks does not include any Unicode-encoded marks (from the ``Combining Diacritical Marks'' ranges), as
these can safely be entered directly. It does include three marks (\textstyleSourceText{cv84[35]},
\textstyleSourceText{[36]} and \textstyleSourceText{[37]}) that lack MUFI code points but are used to form MUFI
characters.

These marks can sometimes be produced by other \textstyleSourceText{cvNN} features, which may be preferable to
\textstyleSourceText{cv84} as providing fallbacks for applications that do not support Character Variant
(\textstyleSourceText{cvNN}) features.
...
that produces them must be applied to both the mark and the base character. For example, if \textstyleSourceText{smcp}
``Small Caps'' changed \textstyleSourceText{cv84[11]} \cvd[10]{84}{◌᷑} to \textstyleSourceText{[12]} \cvd[11]{84}{◌᷑}, it
...
(U+1DD1\slash\textstyleSourceText{cv84[17]}), ◌ͦ (U+0366), ◌ͬ (U+036C), ◌ᷢ (U+1DE2),
◌ͭ (U+036D), ◌ͮ (U+036E), ◌ᷦ (U+1DE6) and \cvd[20]{84}{◌᷑}
(U+1DD1/\textstyleSourceText{cv84[21]}). Instead of being positioned above ascender height as usual (e.g.
\textstyleletterexample{hͣ}), the MUFI glyphs have the marks positioned above the x-height
(e.g. \textstyleletterexample{\addfontfeature{StylisticSet=20}hͣ}).
...
mark, and apply \textstyleSourceText{ss20} to the two glyphs. A variant shape of eth (\textstyleletterexample{{\dh}})
...
\textstyleSourceText{ss20} affects only the diacritics and base characters listed here; other combinations (e.g.
\textstyleletterexample{m\&\_e;}, \textstyleletterexample{\'{h}}) are not affected. It will therefore probably be safe
...
\textstyleSourceText{cv85} -- Variant of ◌ᷓ (U+1DD3, combining open a)}
1=\cvd{85}{◌ᷓ}.

\subsection[48. cv86 {}-- Variant of ◌ᷘ U(U+1DD8, combining insular d)]{%
\stepcounter{Feature}{\theFeature}. \textstyleSourceText{cv86} -- Variant of ◌ᷘ (U+1DD8, combining insular
d)}
1=\cvd{86}{◌ᷘ}.

\subsection[49. cv87 {}-- Variant of ◌ᷣ (U+1DE3, combining r rotunda)]{\stepcounter{Feature}{\theFeature}.
\textstyleSourceText{cv87} -- Variant of ◌ᷣ (U+1DE3, combining r rotunda)}
1=\cvd{87}{◌ᷣ}.

\subsection[50. cv88 {}-- Variant of combining dieresis (U+0308)]{\stepcounter{Feature}{\theFeature}.
\textstyleSourceText{cv8}\textstyleSourceText{8} -- Variant of combining dieresis (U+0308)}
1=\cvd{88}{◌̈}. This also affects precomposed characters on which this variant dieresis may occur, e.g.
\textstyleletterexample{\"a}.

\subsection[51. cv89 {}-- Variant of ◌̅◌ (U+0305, two{}-letter
overline)]{\stepcounter{Feature}{\theFeature}. \textstyleSourceText{cv89} -- Variant of ◌̅◌ (U+0305,
two-letter overline)}
1=\cvd{89}{◌̅◌}.

\subsection[52. cv90 {}-- Variant of ◌̄  (U+0304, combining macron)]{\stepcounter{Feature}{\theFeature}.
\textstyleSourceText{cv}\textstyleSourceText{90} -- Variant of ◌̄ (U+0304, combining macron)}
1=\cvd{90}{◌̄}.

\subsection[53. cv91 {}-- Variants of short horizontal stroke (U+0335)]{\stepcounter{Feature}{\theFeature}.
\textstyleSourceText{cv91} -- Variants of short horizontal stroke (U+0335)}
1=\cvd{91}{◌̵}, 2=\cvd[1]{91}{◌̵}, 3=\cvd[2]{91}{◌̵}. This character can be used with letters with ascenders or
descenders, e.g. \textstyleletterexample{\cvd{91}{d̵ b̵ {\th}̵ p̵}}. \textstyleSourceText{cv91[1]} widens the
stroke, and \textstyleSourceText{cv91[2]} and \textstyleSourceText{[3]} offset the stroke to the right or left. Via
\textstyleSourceText{calt} ``Contextual Alternates,'' this offset is performed automatically for many characters
with ascenders and descenders, and so it should rarely be necessary to use an index with \textstyleSourceText{cv91}.

\subsection[54. cv92 {}-- Variant of breve below (U+032E)]{\stepcounter{Feature}{\theFeature}. cv92 -- Variant of breve
below (U+032E)}
1=\cvd{92}{◌◌̮◌}. Position the mark after the middle of three glyphs, and apply \textstyleSourceText{cv92}
to both the mark and (at least) the middle glyph. This mark is not available via \textstyleSourceText{cv84}.

\section{I. Currency and Weights}
\hypertarget{SectionI}{}\subsection[55. cv93 {}-- Variants of {\textcurrency} (U+0044, generic currency
sign)]{\stepcounter{Feature}{\theFeature}. \textstyleSourceText{cv93} -- Variants of {\textcurrency} (U+0044, generic
currency sign)}
...
\textstyleSourceText{cv9}\textstyleSourceText{4} -- Variant of ℔ (U+2114)}
...
\textstyleSourceText{cv95} -- Variants of {\pounds} (U+00A3, British pound sign)}
...
\textstyleSourceText{ss19} -- Latin to Gothic Transliteration}
Produces Gothic letters from Latin: {\addfontfeature{StylisticSet=1}War{\th} {\th}an in dagans jainans} $\rightarrow $
{\addfontfeature{StylisticSet=19}War{\th} {\th}an in dagans
...
\textstyleSourceText{ss12} -- Early English Futhorc}
...
\subsection[63. ss13 {}-- Elder Futhark]{\stepcounter{Feature}{\theFeature}. \textstyleSourceText{ss13} -- Elder
Futhark}
...
\subsection[64. ss14 {}-- Younger Futhark]{\stepcounter{Feature}{\theFeature}. \textstyleSourceText{ss14} -- Younger
Futhark}
...
\subsection[65. ss15 {}-- Long Branch to Short Twig]{\stepcounter{Feature}{\theFeature}. \textstyleSourceText{ss15} --
Long Branch to Short Twig}
In combination with \textstyleSourceText{ss14}, converts long branch to short twig runes:
{\addfontfeature{StylisticSet=12}{ABCDEFG $\rightarrow $
\addfontfeature{StylisticSet=15}ABCDEFG}}.

\subsection[66. rtlm {}-- Right to Left Mirrored Forms]{\stepcounter{Feature}{\theFeature}. \textstyleSourceText{rtlm}
...
\textstyleSourceText{hlig} -- Historic Ligatures}
...
{\color[rgb]{0.38039216,0.09019608,0.16078432}
f{\textcompwordmark}\"a$\rightarrow $f\"a}

{\color[rgb]{0.38039216,0.09019608,0.16078432}
g{\textcompwordmark}d$\rightarrow $gd}

{\color[rgb]{0.38039216,0.09019608,0.16078432}
g{\textcompwordmark}{\dh}$\rightarrow $\addfontfeature{StylisticSet=1}gð}
...
Notes: For \textstyleletterexample{\textsc{\cvd{99}{\hlig{an}}}} and
\textstyleletterexample{\textsc{\cvd{99}{\hlig{an}}}} see
\hyperlink{RevertSmallCapA}{\textstyleSourceText{cv99}} above. For the ligature \textstyleletterexample{\textsc{\hlig{nſ}}} to
work properly, U+017F \textstyleletterexample{ſ} must be entered directly, not by applying an OpenType feature to
\textstyleletterexample{s}.

\subsection[68. dlig {}-- Discretionary Ligatures]{\stepcounter{Feature}{\theFeature}. \textstyleSourceText{dlig} --
...
\textstyleletterexample{\textcolor[rgb]{0.5529412,0.15686275,0.11764706}{\addfontfeature{Ligatures=Rare}ct, sp, str, st, tr, tt, ty}}.

\subsection[69. ss17 {}-- Rare Digraphs]{\stepcounter{Feature}{\theFeature}. \textstyleSourceText{ss17} -- Rare
Digraphs}
\hypertarget{ss17}{}By ``digraph'' we mean conjoined letters that represent a phonetic value: the most common examples
for western languages are \textstyleletterexample{{\ae}} and \textstyleletterexample{{\oe}} (though these, because they
are so common, are not included in this feature). Use of this feature in web pages enables easier searches: for
example, producing \textstyleletterexample{\addfontfeature{StylisticSet=17}{\th}au} from
\textstyleletterexample{{\th}au} allows the word to be
...
\textstyleletterexample{a} + \textstyleletterexample{\'u} yields \textstyleletterexample{\addfontfeature{StylisticSet=17}a\'u}, and so does
\textstyleletterexample{a} + \textstyleletterexample{u} + U+0301 (combining acute accent). To produce a digraph +
diacritic combination not covered by MUFI (e.g. \textstyleletterexample{ꜵ̀}), you may have to enter the digraph
directly and not via \textstyleSourceText{ss17} (rare diacritics, however, are usually safe to use with this
feature---e.g. \textstyleletterexample{\addfontfeature{StylisticSet=17}aaᷘ}). Note that \textstyleSourceText{ss17} must be applied to the two
...
other features, kerning, and more---include \textstyleSourceText{ccmp} (Glyph Composition/Decomposition),
\textstyleSourceText{calt} (Contextual Alternates), \textstyleSourceText{liga} (Standard Ligatures),
\textstyleSourceText{loca} (Localized Forms), \textstyleSourceText{rlig} (Required Ligatures),
\textstyleSourceText{kern} (Horizontal Kerning), and \textstyleSourceText{mark}/\textstyleSourceText{mkmk} (Mark
...
\textstyleSourceText{ss10} -- Character Entity References}
...
In applications that support Stylistic Sets, \textstyleSourceText{ss10} makes these entities appear as the characters
