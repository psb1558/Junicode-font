...
\textSourceText{options.methodPriority="legible"}, \textSourceText{pua2ot} will instead 
use {\textless}span{\textgreater} elements with inline style
...
\textbf{punctuation}, combining \textbf{marks}, \textbf{currency} signs, and a small 
collection of alphabetic characters (\textbf{alpha})). Select
one or more of the items in this list (use the Shift or Control/Command keys to make
multiple selections) or assign an array to \textSourceText{options.basePreferences}
to alter the substitutions \textSourceText{pua2ot} makes for many characters in these classes. The
default settings are chosen to maximize accessibility and minimize the size of 
